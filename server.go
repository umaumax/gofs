...
func dirList(w http.ResponseWriter, f http.File) {
	...
	for {
		...
		for _, d := range dirs {
			...
			//			fmt.Fprintf(w, `<span><button type="button" class="btn btn-danger" onclick='DELETE(this,%q)'>delete</button> <a href="%s">%s</a></span>`+"\n",
			//	NOTE ディレクトリの場合のボタン仕様を変更?!
			//	NOTE DELETEボタンを表示する用のボタンが必要?! 確認ぐらい欲しい
			fmt.Fprintf(w, `<span>`+
				`<button type="button" class="btn btn-danger"  onclick='DELETE(this,%q)'>delete</button>`+
				`<a      type="button" class="btn btn-primary" href="%s?force_download=true" download=%q target="_blank">download</a>`+
				` <a href="%s" target="_blank">%s</a></span>`+"\n",
				name, url.String(), name, url.String(), htmlReplacer.Replace(name))
		}
	}
	fmt.Fprintf(w, "</pre>\n")
}
...
func serveContent(w http.ResponseWriter, r *http.Request, name string, modtime time.Time, sizeFunc func() (int64, error), content io.ReadSeeker) {
	...
	if r.FormValue("force_download") == "true" {
...
func serveFile(w http.ResponseWriter, r *http.Request, fs http.FileSystem, name string, redirect bool) {
	...
	if d.IsDir() {
		...
		fmt.Fprintf(w, "<html>\n")
		fmt.Fprintf(w, "<head>")
		fmt.Fprintf(w, `<link rel="stylesheet" href="%s" type="text/css" />`, "https://maxcdn.bootstrapcdn.com/bootstrap/3.3.4/css/bootstrap.min.css")
		fmt.Fprintf(w, "<title>%s</title>", "gofs - file server")
		//	for DELETE form
		fmt.Fprintf(w, `<script type="text/javascript" src="%s"></script>`, "http://code.jquery.com/jquery-2.1.4.min.js")
		fmt.Fprintf(w, "</head>")
		fmt.Fprintf(w, "<body>\n")
		//	NOTE "/upload"を変数に
...
function DELETE(e, name) {
	//	[Error] TypeError: Can only call DOMFormData.append on instances of DOMFormData
	//var fd = new FormData();
	//fd.append("name", name);
	$.ajax({
		url: '%s',
		...
		error: function(XMLHttpRequest, textStatus, errorThrown) {
			var text = "";
						text+="XMLHttpRequest : " + XMLHttpRequest.status + "\n";
						text+="textStatus : " + textStatus + "\n";
						text+="errorThrown : " + errorThrown.message + "\n";
			console.log(text);
			$(e).text("error");
		...
		//	create upper dir link
		if filepath.Dir(r.URL.Path) != "/" {
			fmt.Println(r.URL.Path)
			//	upper dir
			name := filepath.Dir(filepath.Dir(r.URL.Path))
			fmt.Fprintf(w, "<pre>\n")
			// name may contain '?' or '#', which must be escaped to remain
			// part of the URL path, and not indicate the start of a query
			// string or fragment.
			...
			//			fmt.Fprintf(w, "<a href=\"%s\">%s</a>\n", url.String(), "back :"+htmlReplacer.Replace(name))
			fmt.Fprintf(w, `<a class="btn btn-default" href="%s" role="button">%s</a>`, url.String(), "back to "+htmlReplacer.Replace(name))
			fmt.Fprintf(w, "</pre>\n")
		}
		dirList(w, f)
		fmt.Fprintf(w, "</body>\n")
